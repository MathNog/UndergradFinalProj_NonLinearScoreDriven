...
OUTPUT
- SparseEstimator """
function subsetSelection(ℓ::LossFunction, Card::Sparsity, Y, X;fixed_idx=Int64[],
    indInit = ind_init(Card, size(X,2), fixed_idx), αInit=alpha_init(ℓ, Y),
    γ = 1/sqrt(size(X,1)),  intercept = false,
    stepRule = poliakRule(1),
  ...
  for restart in 1:numberRestarts
    ...
    for iter in 2:round(Int, maxIter/numberRestarts)
      ...
      n_indices, tie = partial_min!(indices, Card, X, α, γ, cache, fixed_idx)
      ...
      if isa(ℓ, Classification) && norm(∇, 1) == Inf
  ...
  n_indices, = partial_min!(indices, Card, X, a, γ, cache, fixed_idx)
  #Regressor
  w[1:n_indices] = recover_primal(ℓ, Y, X[:,indices[1:n_indices]], γ)
...
function ind_init(Card::Constraint, p::Integer, fixed_idx) 
  non_fixed_idx = findall(i -> !(i in fixed_idx), collect(1:p))
  indices0=vcat(fixed_idx, rand(non_fixed_idx, Card.k-length(fixed_idx)))
  return indices0
end
...
##Minimization w.r.t. s
function partial_min!(indices, Card::Constraint, X, α, γ, cache::Cache, fixed_idx)
  ax = cache.ax
  perm = cache.sortperm
  p = size(X,2)
  ...
  final_idx = deepcopy(fixed_idx)

  count=1
  while length(final_idx) < length(perm)
    if !(perm[count] in final_idx)
      push!(final_idx, perm[count])
    end
    count += 1
  end

  #perm = vcat(fixed_idx, perm_copy_orig[perm_copy][1:n_indices-length(fixed_idx)])

  indices[1:n_indices] = final_idx[1:n_indices]

  sort!(@view(indices[1:n_indices]))

  tie = false
  if n_indices < p
    n_indice_perm = findfirst(i -> i == final_idx[n_indices], perm)
    n_indice_perm_plus1 = findfirst(i -> i == final_idx[n_indices+1], perm)
    if abs(ax[n_indice_perm_plus1] - ax[n_indice_perm])/ax[n_indice_perm] <= 1e-6
      tie = true
      indices[n_indices+1] = final_idx[n_indices+1]
      @warn("possible method error")
    end
  end
