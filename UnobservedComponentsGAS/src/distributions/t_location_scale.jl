...
"
function get_initial_params(y::Vector{Fl}, time_varying_params::Vector{Bool}, dist::tLocationScaleDistribution) where Fl
    ...
    if time_varying_params[2]
        initial_params[2] = (scaled_score.(y ,ones(T) * var(diff(y)), (y.^2) ./ (ones(T) * var(diff(y))) , y, 0.5, dist_code, 2)).^2
    else
