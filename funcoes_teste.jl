...
function denormalize_data(X_norm::Matrix{Fl}, y::Vector{Fl}) where{Fl}
    min = minimum(y)
    max = maximum(y)
    X_orig = deepcopy(X_norm)
    for i in 1:size(X_norm, 2)
        X_orig[:,i] = (X_norm[:,i] .- 0.5) .* (max-min) .+ min
    end
    return X_orig
end
